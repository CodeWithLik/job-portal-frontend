...
const newThead = `<thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Posted Date</th>
                </tr>
              </thead>`;
content = content.replace(oldTheadRegex, newThead);

// Replace tbody cells
// I will replace the specific block inside the map function
const oldTbodyRegex = /<td className="px-6 py-4 whitespace-nowrap">\s*<Link to=\{\`\/recruiter\/applicants\?jobId=\$\{job\.id\}\`} className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800 hover:bg-blue-200 transition-colors cursor-pointer">\s*\{job\.applications_count \|\| 0\} \{job\.applications_count === 1 \? 'Candidate' : 'Candidates'\}\s*<\/Link>\s*<\/td>\s*<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">\s*\{new Date\(job\.created_at\)\.toLocaleDateString\(\)\}\s*<\/td>/;

const newTbodyCells = `<td className="px-6 py-4 whitespace-nowrap text-center">
...
console.log('Updated table alignments in Dashboard.jsx');
